...
// jpe - add new test that does "mgmt endpoints". status, echo, rings, etc
...
func TestTagEndpoints(t *testing.T) {
	util.WithTempoHarness(t, util.TestHarnessConfig{
		Components: util.ComponentRecentDataQuerying | util.ComponentsBackendQuerying | util.ComponentsObjectStorageS3Only, // jpe - restore all backends?
...
func TestTraceByIDandTraceQL(t *testing.T) {
	util.WithTempoHarness(t, util.TestHarnessConfig{
		...
		for _, i := range infos {
			...
			util.SearchTraceQLAndAssertTraceWithRange(t, apiClient, i, now.Add(-time.Hour).Unix(), now.Add(time.Hour).Unix()) // jpe - keep? remove range
...
func TestSearchTagValuesV2_badRequest(t *testing.T) {
	util.WithTempoHarness(t, util.TestHarnessConfig{}, func(h *util.TempoHarness) {
		...
		req, err := http.NewRequest(http.MethodGet,
			fmt.Sprintf("%s/api/v2/search/tag/%s/values", h.APIClientHTTP("").BaseURL, invalidTagName), nil)
		require.NoError(t, err)
