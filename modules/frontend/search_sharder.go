...
func (s *asyncSearchSharder) backendRequests(ctx context.Context, tenantID string, parent pipeline.Request, searchReq *tempopb.SearchRequest, resp *combiner.SearchJobResponse, reqCh chan<- pipeline.Request, errFn func(error)) *combiner.SearchJobResponse {
	...
	blockIter := backendJobsFunc(blocks, s.cfg.TargetBytesPerRequest)
...
func backendJobsFunc(blocks []*backend.BlockMeta, targetBytesPerRequest int) func(shardIterFn, jobIterFn) { // jpe - test
	blocksPerShard := len(blocks) / maxSearchShards // todo: we could weight the initial shards to have fewer blocks. this would make the query faster to first result if search results were common
	...
	return func(shardIterCallback shardIterFn, jobIterCallback jobIterFn) {
		...
		for _, b := range blocks {
			...
			// if jpe jobIterCallBack is nil we can skip the loop and directly calc hte jobsInBlock
			...
			if blocksInShard >= blocksPerShard && currentShard < maxSearchShards {
